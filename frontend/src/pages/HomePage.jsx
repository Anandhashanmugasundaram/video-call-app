...
const HomePage = () => {
  ...
  const [outGoingRequestsIds, setOGoingRequestsIds] = useState(new Set());
  ...
  const { data: outGoingFriendsReq } = useQuery({
    queryKey: ["outGoingFriendsReq"],
    queryFn: getoutGoingFriendsReq,
  });
  ...
  const { mutate: sendRequestMutation, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["outGoingFriendsReq"] });
    },
     onError: (error) => {
    toast.error(
      error?.response?.data?.mssg || "Failed to send friend request."
    );
  },
  });
  ...
  useEffect(() => {
    ...
    if (outGoingFriendsReq && outGoingFriendsReq.length > 0) {
      ...
      setOGoingRequestsIds(outgoingids);
    }
  }, [outGoingFriendsReq]);
            ...
            >
              <div >
                ...
                <p className="opacity-70">
                  {" "}
                  Discover Language exchange partners to learn!
                </p>
              </div>
            </div>
          </div>
